...
namespace AspnetRunBasics.ApiCollection.Infrastructure
{
    public class ApiBuilder
    {
        ...
        {
            var qNsv = HttpUtility.ParseQueryString(_builder.Query);
            qNsv[name] = string.IsNullOrEmpty(qNsv[name]) ? value : string.Concat(qNsv[name], ",", value);

            _builder.Query = qNsv.ToString();
